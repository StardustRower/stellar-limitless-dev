/**
 * 灰岩的状态表：地牢对话和视觉小说共用这一份。
 *
 * Demo 3 把「说话」做成有限状态机。Demo 5 证明同一张表能离开格子，
 * 站进一幕立绘。不要在 vn.js 里再抄一份——改 next / heal，两边一起变。
 *
 * 请分开记：
 * - 本文件：状态、选项、下一跳、交易给哪件物品、治疗几点、本地台词库。
 * - 地牢壳（js/npc.js）：落点、走近、在地图上标异兆。
 * - 视觉小说壳（js/vn.js）：立绘、对话框、选项按钮。
 * - LLM（js/llm.js）：只把「当前状态 + 玩家点了哪一项」写成句子。
 * - 账本（js/memory.js）：只记 met / traded / warned / last_state。
 */
var NpcTable = (function () {
   ...
   * 地牢再额外在地图上标异兆；视觉小说没有格子，只亮旗标。
   */
  function applyWorldEffect(npc, stateObj, world) {
    if (!stateObj || !stateObj.effect) return null;
    var fx = stateObj.effect;
    if (fx.type === "trade") {
      if (npc.traded) return { type: "trade", skipped: true };
      if (!hasSurvey(world.inventory)) return { type: "trade", error: "没有测线残页" };
      var lost = takeSurvey(world.inventory);
      world.inventory.push(fx.itemZh);
      var before = world.hp;
      world.hp = Math.min(world.maxHp, world.hp + fx.heal);
      npc.traded = true;
      return {
        type: "trade",
        beforeHp: before,
  ...
  return {
    ...
    makeNpc: makeNpc,
    localLine: localLine,
    applyWorldEffect: applyWorldEffect,
    mechanicLine: mechanicLine
  };
})();
