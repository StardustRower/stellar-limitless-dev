...
地牢演示没有被这一幕替换。两条 HTML 入口共用 `js/npc-table.js`。改表上的 `next` 或治疗点数，两边一起变。Godot 课把同一张表抄进 Dictionary，见下面 Demo 6。

## Godot 第一间房（Demo 6）

同一张 NPC 表，第一次出现在游戏引擎里。没有走路，没有地牢，不接 FastAPI，不接 LLM。证明「表」能离开浏览器，站进节点树。

1. 安装 **Godot 4.x 标准版**（还没有的话，见 [notes/08-godot-first-scene.md](notes/08-godot-first-scene.md) 逐步说明）。
2. 项目管理器里 **Import**，选中 `godot/huiyan/project.godot`，打开后按 <kbd>F5</kbd>。
3. 左侧是测绘员灰岩的 Polygon2D 剪影（没有图片资源）。右侧是对话框和选项按钮。
